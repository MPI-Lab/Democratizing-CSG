...
class ValidationAnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # print(device)
        validation_image_id_ante_embedding = validation_image_id_ante_embedding.to(device, dtype=image_embeddings.dtype)
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    pose_latents = self.pose_net(pose_pil_image_list[idx].to(device, dtype=latent_model_input.dtype))
