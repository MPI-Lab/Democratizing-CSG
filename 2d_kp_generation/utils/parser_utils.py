...
def parse_and_load_from_dataset():
    ...
    group.add_argument("--dataset_config", default="data_loaders.nba.config.NBA_CONFIG", type=str, help="Dataset name (choose from list).")
    update_defaults(group, parse_and_load_from_path("model_path", "resume_checkpoint", "evaluator_path"))
    dataset_config = parser.parse_known_args()[0].dataset_config
    return vars(get_dataset_config(dataset_config))
...
def add_model_options(parser: ArgumentParser):
    ...
    update_defaults(group, parse_and_load_from_path("model_path"))
...
def add_data_options(parser: ArgumentParser):
    group = parser.add_argument_group("dataset")
    group.add_argument("--dataset_config", default="data_loaders.nba.config.NBA_CONFIG", type=str, help="dataset config import path")
    ...
    group.add_argument("--cond", default="no_cond", type=str, help="Type of conditioning to use.")
    update_defaults(group, parse_and_load_from_path("model_path", "resume_checkpoint", "evaluator_path"))
...
def add_training_options(parser: ArgumentParser):
    ...
    group.add_argument("--loss_weight", default="mean", type=str, choices=["mean", "extra_mouth", "extra_mouth_hand"], help="Loss weight type.")
    group.add_argument("--if_train_vis", action="store_true", help="If True, will visualize training samples.")
    group.add_argument("--if_val_vis", action="store_true", help="If True, will visualize validation samples.")
    group.add_argument("--save_sample_progressive", action="store_true", help="If True, will visualize progressive samples during training.")
...
def add_sampling_options(parser: ArgumentParser):
    ...
    group.add_argument("--audio_folder", default=None, type=str, help="Path to audio folder.")
    group.add_argument("--kp_folder", default=None, type=str, help="Path to kp folder.")
    group.add_argument("--motion_length", default=None, type=str, help="The length of the sampled motion in seconds.")
