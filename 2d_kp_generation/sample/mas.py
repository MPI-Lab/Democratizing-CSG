...
from utils.plot_script import plot_3d_motion, plot_motion
...
class MAS(DiffusionSampler):
    ...
    def visualize_3d(self, motions_3d: torch.Tensor, title=""):
        ...
        for i in range(self.num_samples):
            ...
            save_path = os.path.join(self.args.output_dir, f"{title}_{i}_{self.t}")
    ...
    def visualize_multiview_2d(self, sample, title="", normalized=False, model_kwargs=None, use_mask=False):
        if normalized:
            sample = self.inverse_transform(sample)
        model_kwargs = self.model_kwargs if model_kwargs is None else model_kwargs
        sample = sample.cpu().numpy()
        for view in range(sample.shape[0]):   
            for sample_i in range(sample[view].shape[0]):
                ...
                motion = sample[view][sample_i].transpose(2, 0, 1)[:length]
                save_path = os.path.join(self.args.output_dir, f"{title}_{sample_i}_{self.t}_{view}")
    ...
    def optimize(self, samples):
        ...
        for i in range(OPTIMIZE_STEPS):
            ...
            logging.info(f"---optimizing 3d motions step: {i}---")
            logging.info(f"---loss sample: {loss_sample}---")
    ...
    def __call__(self, model_kwargs=None, save=True, visualize=True, progress=True):
        ...
        for i in range(10):  # temporary
            logging.info(f"---inference batch {i}---")
            ...
            for self.t in (tqdm(self.schedule) if progress else self.schedule):
                ...
                logging.info(f"---denoising step {self.t}---")
                ...
                if (self.t % 30) == 0:
                    # intermidiate 2d visualization before optimization
                    self.visualize_multiview_2d(self.samples, f"result_intermidiate_2d_before_{i}")
                    
                    # intermidiate 3d visualization
                    self.visualize_3d(self.motions_3d, title=f"result_intermidiate_3d_{i}")                
                    
                    # intermidiate 2d visualization after optimization
                    self.visualize_multiview_2d(self.projected_motions, f"result_intermidiate_2d_after_{i}")


                self.x_t = self.sample_xt(self.transform(self.projected_motions), self.x_t, self.t)

            if save:
                self.save_motions(motions=self.motions_3d, title=f"result_{i}")
            if visualize:
                self.visualize_3d(self.motions_3d, title=f"result_{i}")
