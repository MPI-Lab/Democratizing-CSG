...
class DiffusionTrainer(Trainer):
    ...
    def sample_with_cfg(self, x_T, schedule, cond, scale):
        ...
        uncond["use_learnable_uncond"] = True
        uncond["scale"] = torch.full((x_T.shape[0],), scale, device=self.device)
        x_t = x_T
        sample_progressive = {}
        for t in tqdm(schedule):
            t_batch = torch.full((x_t.shape[0],), t, dtype=torch.long, device=self.device)
            ...
            sample = sample_uncond + (uncond["scale"].view(-1, 1, 1, 1) * (sample - sample_uncond)) 
    ...
    def save_conditions(self, cond, save_num, save_path, prefix, combine_videos_num):
        combine_videos_num += 1 # gt videos
        ...
        for i in range(save_num if save_num < len(cond['y']['filenames']) else len(cond['y']['filenames'])):
            filename = prefix + cond["y"]["filenames"][i] + cond["y"]["filenames_reference"][i]
            ...
            video_cut_command = f"ffmpeg -y -ss {global_start_time} -i {filepath['video']} -t {length_time} -c:v libx264 -c:a aac {os.path.join(save_path, 'mp4', filename + '.mp4')}"
            ...
            if "initial_frames" in cond["y"]["conditions"]:
                start_frame_ref = cond["y"]["conditions"]["start_frame_ref"][i]
                ...
                # shutil.copy(filepath["video"], os.path.join(save_path, "mp4", filename + ".mp4"))
                duration = self.test_data.dataset.initial_frames_num / self.test_data.dataset.fps
                video_cut_command = f"ffmpeg -y -ss {start_time_ref} -i {filepath_ref['video']} -t {duration:.2f} -c:v libx264 -c:a aac {os.path.join(save_path, 'mp4_initial_frames', filename + '.mp4')}"
                os.system(video_cut_command)
            ...
            if "initial_kp" in cond["y"]["conditions"]:
                ...
                duration = 1 / self.test_data.dataset.fps
                video_cut_command = f"ffmpeg -y -ss {start_time_ref} -i {filepath_ref['video']} -t {duration:.2f} -c:v libx264 -c:a aac {os.path.join(save_path, 'mp4_initial_kp', filename + '.mp4')}"
    ...
    def save_combined_vis(self, motion, cond, save_num, save_path, prefix, combine_videos_num):
        for i in range(save_num if save_num < motion.shape[0] else motion.shape[0]):
            ...
            combine_command = f"""ffmpeg -i {os.path.join(save_path, 'mp4', filename + '.mp4')} \
                    ...
                    {"-i " + os.path.join(save_path, 'mp4_initial_frames', filename + '.mp4') if "initial_frames" in cond["y"]["conditions"] else ""} \
    ...
    def _test_dataloader(self):
        self.step += 302494 
        for epoch in itertools.count(start=323):
            if self.use_ddp:
            ...
            for motion, cond in iter_train:
                if self.step > 302494:
                    ...
                    for i in range(len(cond['y']['filenames'])):
                        filename = f"{self.step}_" + cond["y"]["filenames"][i] + cond["y"]["filenames_reference"][i]
                        filepath = cond["y"]["filepaths"][i]
                        filepath_ref = cond["y"]["filepaths_reference"][i]
                        ...
                        # save video
                        save_path = "/apdcephfs_cq8/share_1330077/bonoxyang/tencent-cospeech/tencent-cospeech/2d_kp_generation_fixed/save_test/test_320h_data"
                        os.makedirs(os.path.join(save_path, "mp4"), exist_ok=True)
                        # shutil.copy(filepath["video"], os.path.join(save_path, "mp4", filename + ".mp4"))
                        video_cut_command = f"ffmpeg -y -ss {global_start_time} -i {filepath['video']} -t {length_time} -c:v libx264 -c:a aac {os.path.join(save_path, 'mp4', filename + '.mp4')}"
                        os.system(video_cut_command)
                        ...
                        # save GT's motion visualization
                        os.makedirs(os.path.join(save_path, "gt_pose_vis"), exist_ok=True)
                        visualize_motion_with_audio(motion=vis_gt, audio_path=None, start_time=-1,
                                                    title="GT", filename=filename, save_path=os.path.join(save_path, "gt_pose_vis", filename + ".mp4"),
                                                    skeleton_links=self.test_data.dataset.skeleton_links,
                                                    skeleton_links_colors=self.test_data.dataset.skeleton_links_colors)
    ...
    def sample_and_visualize_unpaired(self, motion, cond, save_num, save_path, prefix="", scale=1):
        ...
        # original_filenames = cond_unpaired["y"]["filenames"]
        # new_filenames = [original_filenames[i] + '_' + original_filenames[random_order[i]] for i in range(len(original_filenames))]
        # cond_unpaired["y"]["filenames"] = new_filenames
        ...
        cond_unpaired["y"]["conditions"]["start_frame_ref"] = [cond_unpaired["y"]["conditions"]["start_frame_ref"][i] for i in random_order]

        if "initial_frames" in cond_unpaired["y"]["conditions"]:
            cond_unpaired["y"]["conditions"]["initial_frames"] = cond_unpaired["y"]["conditions"]["initial_frames"][random_order]

        if "initial_kp" in cond_unpaired["y"]["conditions"]:
            cond_unpaired["y"]["conditions"]["initial_kp"] = cond_unpaired["y"]["conditions"]["initial_kp"][random_order]
    ...
    def sample_and_visualize_long(self, motion, cond, save_num, save_path, prefix="", scale=1):
        ...
        cond_long['y']['conditions']['start_frame_ref'] = long_video_condition['start_frame_ref']
...
class DiffusionTrainerEval(DiffusionTrainer):
    ...
    def evaluate(self):
        if self.args.eval_during_training:
            ...
            # self.model.eval()
            # with torch.no_grad():
            #     for i, (motion, cond) in enumerate(self.data):
            #         motion = motion.to(self.device)
            #         cond = model_kwargs_to_device(cond, self.device)
            #         sample = sample_fn(
            #                     self.model,
            ...
            #         sample = sample.cpu().numpy().transpose(0, 3, 1, 2) # (bs, seqlen, J, Jdim)
            #         os.makedirs(os.path.join(self.args.save_dir, f"checkpoint_{self.step}"), exist_ok=True)
            #         batch_num = len(sample)
            #         idx = np.random.choice(batch_num, 1)
            #         np.save(os.path.join(self.args.save_dir, f"checkpoint_{self.step}", f"sample_{self.step}_{i}_{idx}.npy"), sample[idx].squeeze(0))
            #         # debug
            #         if i >= 0:
            #             break
            # self.model.train()
